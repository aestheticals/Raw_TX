...
uncompressed_pub_key = str("04") + str(hex(xPublicKey)[2:-1]) + str(hex(yPublicKey)[2:-1])
print "X coordinate of uncompressed pub key: "
print str(hex(xPublicKey)[2:-1])
print "Y coordinate of uncompressed pub key: "
print str(hex(yPublicKey)[2:-1])
print

if(int(yPublicKey) % 2 == 0): first_byte = "02"
else: first_byte = "03"
compressed_pubkey = first_byte + hex(xPublicKey)[2:-1]
...
sha1.update(compressed_pubkey.decode('hex'))
hash1 = sha1.digest()
...
public_address = base58.b58encode(hash4.decode('hex'))
...
prev_tx_raw_input = ""
double_sha.update( prev_tx_raw_input.decode('hex') )
...
prev_tx_hash = double_sha2.digest().encode('hex')
print
...
address_to_send_to = "Vrz9oQy69zyMXWZZJ47BkWUhCXnJ4bjCXu"
output_scriptpubkey = base58.b58decode(address_to_send_to).encode('hex')
...
input_script_len = str(hex(len(input_scriptpubkey)/2))[2:]
...
output_script_len = str(hex(len(output_scriptpubkey)/2))[2:]
output_scriptpubkey
...
hashof_signing_message = hashlib.sha256(hashlib.sha256(signing_message.decode('hex')).digest()).hexdigest()
#convert to decimal
...
r_len = hex(len(r_coor)/2)[2:]
s_len = hex(len(s_coor)/2)[2:]
